...
export default function Produtos(){
  ...
  function criar(e){
    ...
    fetch('/api/cidades/0', { method: form.method, body: formJson, headers: {'Content-Type': 'application/json', 'id':'1', 'tk':'7cea26600c288a7055229a1d7e9ba49b' } }) 
    .then((response) => {
      if(response.ok){
  ...
  const apagar = async idP =>{
    if (confirm("Deseja apagar a cidade "+idP+" ?") == true) {
      const response = await fetch(`/api/cidades/${idP}`, { method: 'DELETE', headers: { 'id':'1', 'tk':'7cea26600c288a7055229a1d7e9ba49b' } } );
