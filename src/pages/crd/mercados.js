...
export default function Produtos(){
  ...
  function criar(e){
    ...
    fetch('/api/mercados/0', { headers: {'Content-Type': 'application/json', 'id':'1', 'tk':'7cea26600c288a7055229a1d7e9ba49b'}, method: form.method, body: formJson })
    .then((response) => {
      if(response.ok){
  ...
  const apagar = async idP =>{
    if (confirm("Deseja apagar o mercado "+idP+" ?") == true) {
      const response = await fetch(`/api/mercados/${idP}`, { method: 'DELETE', headers: {'id':'1', 'tk':'7cea26600c288a7055229a1d7e9ba49b'} });
